...
<div className=" p-3  space-y-4 rounded-xl w-full pt-12  min-h-64">
                            {data.map((d, index) => (
                                ...
                                <span className=" uppercase  text-gray-700 font-semibold text-sm dark:text-gray-50 min-w-24">{d.total} {d.name}</span>
                                    ...
                                    <div className=" flex gap-2 duration-150 h-16 w-full"  >
                                      {d.justified !== 0 && <div className="bg-emerald-300 items-center justify-center flex dark:bg-green-300 duration-500 rounded-md h-full text-lg font-bold text-emerald-900" style={{ width: `${d.justified * 100 / d.total}%`}} >
                                         {d.justified && d.justified}
                                      </div>}
                                     {d.notJustified !== 0 && <div className="bg-yellow-300 flex items-center justify-center dark:bg-yellow-300 duration-500 rounded-md h-full text-lg font-bold text-yellow-900" style={{ width: `${d.notJustified * 100 / d.total}%`}} >
                                         {d.notJustified }
                                      </div>}
                    ...
                    <div className=" absolute bottom-7 flex items-center justify-center w-full gap-5 mt-10 lg:mt-5">
