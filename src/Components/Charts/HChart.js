                ...
                {data.map((d, index) => (
                    <div key={index} className=" flex flex-row gap-1  justify-end items-center  h-full w-full " >
                        ...
                        <div className=" flex gap-2 duration-150 h-16 w-full"  >
                            {d.justified !== 0 && 
                                <div className="bg-emerald-300 items-center justify-center flex dark:bg-green-300 duration-500 rounded-md h-full text-lg font-bold text-emerald-900" style={{ width: `${d.justified * 100 / d.total}%`}} >
                                    {d.justified && d.justified}
                                </div>
                            }
                            {d.notJustified !== 0 && 
                                <div className="bg-yellow-300 flex items-center justify-center dark:bg-yellow-300 duration-500 rounded-md h-full text-lg font-bold text-yellow-900" style={{ width: `${d.notJustified * 100 / d.total}%`}} >
                                    {d.notJustified }
                                </div>
                            }
                        </div>             
